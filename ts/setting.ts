...
document.querySelector('#pure-save')?.addEventListener('click', (event) => {
    ...
    jQuery.ajax({
        //@ts-ignore
        url: obvInit.ajaxurl,
        data: data + '&action=berry_setting',
        type: 'POST',
        success: function () {
            //@ts-ignore
            const html = `<div id="berry-settings_updated" class="notice notice-success settings-error is-dismissible"><p><strong>${obvInit.success_message}</strong></p><button type="button" class="notice-dismiss"></button></div>`;
            //@ts-ignore
...
+(function ($) {
    $(document).on('click', '#berry-settings_updated .notice-dismiss', function () {
        $('#berry-settings_updated').remove();
    });
