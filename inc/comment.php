...
class berryComment
{
    ...
    {
        register_rest_route('berry/v1', '/comment', array(
            'methods' => 'POST',
            'callback' => array($this, 'handle_coment_post'),
            'permission_callback' => '__return_true',
        ));

        register_rest_route('berry/v1', '/view', array(
            'methods' => 'get',
            'callback' => array($this, 'handle_post_view'),
            'permission_callback' => '__return_true',
        ));

        register_rest_route('berry/v1', '/like', array(
            'methods' => 'POST',
            'callback' => array($this, 'handle_post_like'),
            'permission_callback' => '__return_true',
        ));

        register_rest_route('berry/v1', '/archive/(?P<id>\d+)', array(
            'methods' => 'POST',
            'callback' => array($this, 'handle_archive_view'),
            'permission_callback' => '__return_true',
        ));

        register_rest_route('berry/v1', '/posts', array(
            'methods' => 'get',
    ...
    function get_comment_author_hack($comment_author, $comment_id, $comment)
    {
        ...
        if ($comment->user_id == $post->post_author) {
            $comment_author = $comment_author . '<span class="comment--author__tip">' . __('Author', 'Berry') . '</span>';
        }
        return $comment_author;
    }
    ...
    function handle_posts_request($request)
    {
        ...
        while ($the_query->have_posts()) {
            ...
            $data[] = [
                ...
                'view_count' => (int)get_post_meta(get_the_ID(), BERRY_POST_VIEW_KEY, true),
                'like_count' => (int)get_post_meta(get_the_ID(), BERRY_POST_LIKE_KEY, true),
                // 'thumbnail' => berry_get_background_image(get_the_ID(), 300, 200),
                'permalink' => get_permalink(),
                'categories' => get_the_category(),
                'tags' => get_the_tags(),
                // 'has_image' => berry_is_has_image(get_the_ID()),
                'day' => get_the_date('d'),
    ...
    function handle_archive_view($request)
    {
        ...
        $views = (int)get_term_meta($request['id'], BERRY_ARCHIVE_VIEW_KEY, true);
        $views++;
        update_term_meta($request['id'], BERRY_ARCHIVE_VIEW_KEY, $views);
        return [
            'code' => 200,
            'message' => __('Success', 'Berry'),
            'data' => $views
    ...
    {
        $post_id = $data['id'];
        $post_views = (int)get_post_meta($post_id, BERRY_POST_VIEW_KEY, true);
        $post_views++;
        update_post_meta($post_id, BERRY_POST_VIEW_KEY, $post_views);
        return [
            'code' => 200,
            'message' => __('Success', 'Berry'),
            'data' => $post_views
    ...
    {
        $post_id = $request['id'];
        $post_views = (int)get_post_meta($post_id, BERRY_POST_LIKE_KEY, true);
        $post_views++;
        update_post_meta($post_id, BERRY_POST_LIKE_KEY, $post_views);
        return [
            'code' => 200,
            'message' => __('Success', 'Berry'),
            'data' => $post_views
...
// comment template
function berry_comment($comment, $args, $depth)
